...
todos:
  ...
  - id: mix-audio
    ...
    status: pending
  - id: operator-gui
    content: Desktop GUI を Mix/Swap、PGM直指定、Overlay追加/Scene割当、Mixing Unit追加、Audio Follow、MixFeed入れ子、未知Overlayエラーへ実配線する
    status: completed
  - id: omt-vertical-slice
    content: openmediatransport-rsで discovery/receive/send、映像・音声、tally/metadata、再接続をRuntime/GUIへ実配線し相互運用試験を通す
    status: pending
  - id: remaining-io
...
- Rust 2024 edition、Windows first、wgpu中心のnative GUI applicationとします。公開版 `gpu-video 0.4.0` はwgpu 29、upstreamはさらに新しく、単一wgpu 25 Device/Queueと型・所有権を共有できません。ADR-0015でfake feature/probeを削除し、互換adapterとHILを伴う将来提案まで非採用としました。
- Smelter本体は製品組込みに商用ライセンスが必要なため非採用です。合成はeiviz独自wgpu compositor、encodeは明示software/external `ProgramEncoderFactory`を使用します。
...
| 領域 | 到達段階 | 未達 |
|---|---|---|
| Core/Time/Command | Runtime wired（整数 affine ClockMapper/TimingIsland、SourceMedia/DeckLinkStream/Genlock/AudioSample/PTP/Monotonic domain、bounded drift/offset、jump/wrap/reset/lock診断、明示clock policy、monotonic deadline/A/V drift metrics、bounded effective-time schedule、frame-boundary immutable latch、command flood/replay） | TIME-HIL-01..08、wall-clock multi-client/audio callback HIL証跡 |
| Project | Runtime/GUI wired（atomic command rollback、asset ingest、save/load、portable export/import、path traversal/hash spoof拒否、決定論corrupt/truncate/fuzz-like/disk-write fault試験、欠落asset診断、hardware ID再割当） | 別PC/24h、実device hot-plug HIL |
| Compositor | Runtime wired（実WGSL layer/transition合成、単一Device/Queue、native preview texture、bounded prewarm/resource pool、loss hard-degrade、frame-boundary reinjection、明示readback計数） | eframe render-state再生成を含む実device loss、NVIDIA/AMD/Intel HIL/P99 |
| File media | Runtime/GUI wired（PNG/JPEG、Shiguredo MP4、明示OpenH264 + FDK AAC-LC、mp4a/esds ASC、edit-list priming、共有timeline、seek/loop同期reset、明示ExactRate/ASRC、video-only/A/V表示、colr/nclxまたはH.264 VUIの明示BT.709 limited照合） | 代表clip A/V skew、codec conformance、cross-platform/long-loop HIL |
| Mixing/Audio/Output | Runtime wired（OutputごとのProgram/Multiview source選択、DAG/Follow/delay/pan、bounded sinc ASRC/drift/reset診断、CPAL callback静的realtime guard）。Desktop operator GUI は Mix/Swap TAKE、PGM直指定、Overlay追加/Scene割当、Mixing Unit追加、Audio Follow、MixFeed入れ子（下段UnitはTAKE対象ではなく MixFeed 入力として上位Unitへ載せる）まで実配線。未知 Overlay は silent no-op ではなく `UnknownId` | 実device xrun/drift/24h HIL。HIL未達のため mix-audio Phase は未完了 |
| OMT | Runtime/GUI wired（pure-Rust openmediatransport-rs/vmx-rs、BGRA/UYVY、映像・音声receive/send、bounded metadata、visibility連動tally、reconnect診断、GUI接続、project load時の明示OMT再bind、`OmtSink`→`OmtSource` localhost protocol loopback、`Engine::bind_omt_source` の Program 実画素試験） | 別PC reference-tool interop、network fault/24h実証。localhost loopback は OMT-HIL 合格にしない |
| NDI | Runtime/GUI wired（grafton-ndi 1.0.0、discovery、映像・音声receive/send、metadata/sender tally、NV12 BT.709、reconnect診断、Desktop接続、project load時の明示NDI再bind、SDK/feature未導入時はAdmission拒否で代替なし） | NDI 6 SDK/runtime build、別PCinterop、receiver tally API非提供の制約確認、network fault/24h実証。SDK無し試験は NDI-HIL 合格にしない |
| DeckLink/Audio I/O | Runtime/GUI wired（SDK16 C++ shim capture/scheduled playback、CPAL WASAPI/CoreAudio/ALSA/PipeWire、explicit ASIO、明示ASRC、project load時の明示DeckLink/audio再bind、SDK/feature未導入時はAdmission拒否で代替なし） | SDK/hardware build、genlock/xrun/24h HIL。SDK無し試験は DL-HIL/AUD-HIL 合格にしない |
| Encode/stream | Runtime/GUI wired（Desktop mapping→Engine→encoder→shared AU fanout、RTMP/SRT/fMP4、reconnect/recovery、project load時は live encoder session を破棄して stopped のまま残し、明示Start+encoder binary 以外では開始しない。PCM/I_PCM 代替なし） | production binary HIL、real-server/loss/24h。localhost/mock encoder は DIST-HIL 合格にしない |
| Control | Runtime wired（versioned HTTP/TCP/WebSocket Query・Command、全transport auth/rate/body/frame/bounded queue、revision event、atomic transaction、optional midir実入力とDesktop明示mapping） | physical MIDI HIL、TLS proxy/別PC、multi-client flood/soak証跡 |
| Certification | Automated evidence harness（24/72h virtual equivalent、wall-clock mode、Program/deadline/A-V assertions、max graph、command flood/replay、sink/disk/NIC fault、platform memory、Miri/ASan/Loom、structured JSON/matrix）＋unsigned MSIX/pkg/deb・rollback・secret-gated signing workflow | Windows実機、GPU/SDK HIL、24–72h wall-clock、実key署名/notarization。simulation は HIL 合格にしない |
...
- `R07 I/O`：NDI送受信は`grafton-ndi`、OMTはpure-Rust `openmediatransport-rs`、DeckLinkは公式SDK用の最小C++/C ABI shim、audioはCPALのASIO/WASAPI/CoreAudio/ALSA系backendとして隔離します。
...
- `crates/eiviz-codec-*`、`crates/eiviz-io-*`：software/external codec、NDI、OMT、DeckLink、CPAL/ASIOを個別crate/featureに隔離します。互換実装のないgpu-video stubは置きません。
...
1. **Phase 0 — 要件凍結・PoC・法務ゲート**
   ...
   - Smelter/gpu-video調査結果はADR-0002/0015で固定済み。今後のcodec、egui texture sharing、NDI/OMT/DeckLink/ASIOのbuild/distributionを独立spikeで計測します。
...
6. **Phase 5 — AV-over-IP adapters**
   ...
   - OMTはMIT `openmediatransport-rs` + `vmx-rs`を使用し、libomt C ABI経路は残しません。別PC reference toolとの相互運用証跡が揃うまでPhase完了にしません。
...
8. **Phase 7 — Encode・stream・record**
   - 明示software/external codec profile、RTMP/SRT/fMP4、keyframe/reconnect/segment recovery、sink isolationを実装します。暗黙fallbackは禁止です。
...
10. **Phase 9 — Cross-platform・追加映像profile・release認定**
   ...
   - `eiviz-certification` の short deterministic smoke は public CI、24/72h
     wall-clock と hardware/SDK scenario は manual/private runner に分離します。
     evidence JSON は requirement ID/commit/OS/profile/test と artifact path を保持し、
     hardware attestation 無しの simulation を HIL pass に昇格しません。
