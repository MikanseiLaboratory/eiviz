...
| Project | Runtime/GUI wired（atomic command rollback、asset ingest、save/load、portable export/import） | 完全migration、別PC、corrupt/disk-full/fuzz/24h |
| Compositor | Runtime wired（実WGSL layer/transition合成、明示staging readback） | eframeとの単一Device統合、texture pool、device-loss recovery、NVIDIA/AMD/Intel HIL/P99 |
| File media | Runtime/GUI wired（PNG/JPEG、Shiguredo MP4、明示OpenH264 + FDK AAC-LC、mp4a/esds ASC、edit-list priming、共有timeline、seek/loop同期reset、明示ExactRate/ASRC、video-only/A/V表示） | MP4色metadata照合、代表clip A/V skew、codec conformance、cross-platform/long-loop HIL |
...
| Certification | Automated evidence harness（24/72h virtual equivalent、wall-clock mode、Program/deadline/A-V assertions、max graph、command flood/replay、sink/disk/NIC fault、memory/queue high-water、structured JSON、R01–R11/AC-01–11 matrix） | Windows実機、GPU/SDK HIL、24–72h wall-clock、installer/signing。simulation は HIL 合格にしない |
