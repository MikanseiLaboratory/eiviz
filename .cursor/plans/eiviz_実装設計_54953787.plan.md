...
| Compositor | Runtime wired（実WGSL layer/transition合成、eframeとの単一Device/Queue統合、native preview texture、明示staging readback） | texture pool、device-loss recovery、NVIDIA/AMD/Intel HIL/P99 |
...
| Mixing/Audio/Output | Runtime wired（OutputごとのProgram/Multiview全体source選択、OutputKind/MediaSink/encode route、Desktop controls、DAG/Follow/delay/pan、CPAL callback静的realtime guard） | production ASRC、実device 24h HIL |
| OMT | Runtime wired（pure-Rust openmediatransport-rs/vmx-rs、映像・音声receive/send、GUI接続） | 別PCinterop、tally/metadata surfacing、UYVY、reconnect/network fault実証 |
| NDI | Runtime wired（grafton-ndi 1.0.0、discovery、映像・音声receive/send、Desktop接続） | NDI 6 SDK/runtime環境でのbuild、別PC/reference tool interop、tally/metadata、reconnect・network fault実証 |
| DeckLink/Audio I/O | Runtime wired（SDK16 C++ shim capture/scheduled playback、CPAL WASAPI/CoreAudio/ALSA/PipeWire、explicit ASIO） | SDK/hardware build、genlock/xrun/24h HIL、ASRC |
...
| Certification | Automated evidence harness（24/72h virtual equivalent、wall-clock mode、Program/deadline/A-V assertions、max graph、command flood/replay、sink/disk/NIC fault、Linux/Windows/macOS platform API memory、Miri/ASan/Loom、structured JSON、matrix） | Windows実機、GPU/SDK HIL、24–72h wall-clock、installer/signing。simulation は HIL 合格にしない |
