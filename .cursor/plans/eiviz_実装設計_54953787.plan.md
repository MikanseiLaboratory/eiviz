...
| Project | Runtime/GUI wired（atomic command rollback、asset ingest、save/load、portable export/import、欠落assetのpath/期待・実SHA-256/policy診断、列挙済みhardware IDへのversioned atomic再割当） | 完全migration、別PC、corrupt/disk-full/fuzz/24h、実device hot-plug HIL |
...
| Mixing/Audio/Output | Runtime wired（unit別Output、Multiview、DAG/Follow/delay/pan、CPAL input/output callback） | Multiview個別output、production ASRC、実device 24h HIL |
...
| Encode/stream | Adapter/GUI wired（shared AU fanout、RTMP/SRT/fMP4、reconnect/recovery） | production AVC/AAC encoder、Engine tick encode/publish、real-server/loss/24h |
...
| Certification | Automated evidence harness（24/72h virtual equivalent、wall-clock mode、Program/deadline/A-V assertions、max graph、command flood/replay、sink/disk/NIC fault、memory/queue high-water、structured JSON、R01–R11/AC-01–12 matrix） | Windows実機、GPU/SDK HIL、24–72h wall-clock、installer/signing。simulation は HIL 合格にしない |
