...
| File media | Runtime/GUI wired（PNG/JPEG）、H.264 avc1 strict MP4 index/AVCC/sync-seek境界実装済み | OpenH264 dynamic decode、play/pause/seek/loop、AAC/A/V sync |
| Mixing/Audio/Output | 一部Runtime wired（unit別Program→OutputId→sink、DAG/Follow/delay/pan、複数Multiview Input/PRV/PGM描画） | Multiview個別出力adapter、production ASRC、device callback、24h |
...
| DeckLink/ASIO | Model/stub only | SDK実装、Runtime配線、HIL |
