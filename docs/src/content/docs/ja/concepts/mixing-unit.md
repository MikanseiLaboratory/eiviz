...
Add Inputから、このユニットのPreviewまたはProgramをMix Inputとして追加できます。既存のFrameDelayリング（1–8フレーム）を参照し、swapchainは増やしません。別のMixing Unitに載せると入れ子のM/Eになります。同じユニットへの自己配線は拒否されます。
