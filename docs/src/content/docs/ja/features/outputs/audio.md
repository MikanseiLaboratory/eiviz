...
WindowsではWASAPI共有モードでマイク、出力ループバック、Application Audioを取り込みます。デバイスを空にすると、その時点の既定デバイスを追従します。排他モードが失敗しても共有へ黙って落ちません。ASIO入力は出力バスと同じドライバインスタンスを共有し、1+2、3+4のようにステレオ対で追加します。macOSのCore Audio入力は未実装で、選んだ場合は明示的に失敗します。
