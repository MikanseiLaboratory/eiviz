...
Saveボタンはホストが保持している現在のセッションファイルへ書き込みます。Remoteはパスもファイル名も送りません。保存先はホスト側に既にあるファイルです（GUIのSave、または`eiviz-headless run --session`）。ホスト側で一度も保存していない場合はエラーになります。成功時は保存先パスと、ファイル内に残っている履歴件数を表示します。直前のドキュメントは`.eivz`内に残ります（最大20件）。Exportファイル（`.eivzx`）に履歴は含まれません。
