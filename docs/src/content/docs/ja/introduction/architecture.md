...
シーンタイル、スイッチャーのシーンサムネ、入力プレビューはGPUから読み戻したサムネです。シーンやMix Inputを増やしてもswapchainは増えません。Mix InputはMixing UnitバスまたはセッションMultiviewの遅延エイリアスで、FrameDelayリングを読み、同じサムネ経路を使います。
