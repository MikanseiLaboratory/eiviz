...
A Mix Input is a delayed reroute of an existing bus. It reads N frames ago from the FrameDelay ring and does not add a swapchain. The same Mixing Unit cannot use a Mix Input that targets itself. Nested M/E (Mix of unit A on unit B) is stable with that delay. Session Multiview Mix Inputs start muted.
