...
Session files are owned by the mixer. Ordinary save is `.eivz` (in-file history, no embedded media). Export is `.eivzx` (embedded Still/Video, empty history). Both open from the same native file dialog. Load Last opens the most recent existing file. A file saved on one OS loads as the same session on another.
