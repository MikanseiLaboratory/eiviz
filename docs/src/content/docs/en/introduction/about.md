...
Session files are owned by the mixer. Ordinary save is `.eivz` (in-file history, no embedded media). Export is `.eivzx` (embedded Still/Video, no history). Both open from the same native file dialog. A file saved on one OS loads as the same session on another.
