...
public partial class SettingsWindow : Window
{
    ...
    private void MvList_SelectionChanged(object sender, SelectionChangedEventArgs e) => FillSelectedMultiview();
    ...
    private void MvOnTop_Click(object sender, RoutedEventArgs e)
    {
        if (_suppressMv || MvList.SelectedItem is not MultiviewLayout layout)
            return;
        layout.AlwaysOnTop = MvOnTopBox.IsChecked == true;
        if (Owner is MainWindow main)
            main.ApplyMultiviewTopmost(layout);
    }

    private void MvLabelAnchor_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (_suppressMv || MvList.SelectedItem is not MultiviewLayout layout)
            return;
